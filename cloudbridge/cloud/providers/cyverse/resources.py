...
class CyverseClient(object):
    ...
    def __repr__(self, show_extra=True, show_secret=False):
        extraneous = ""
        if self.extraneousParameters and show_extra:
            extraneous = "\n\t- extra info:: "
            for e in self.extraneousParameters:
                extraneous = extraneous + "{} : {}, ".format(
                        e, self.extraneousParameters[e])
            extraneous = extraneous + "\n|CyverseClient"
        if show_secret:
            secret = self.consumerSecret
        else:
            secret = "<redacted>"
        return ("<CyverseClient| name: {}, callback: {}, description: {}"
                " consumerKey: {} consumerSecret: {} {}/>").format(
                    self.clientName, self.callbackUrl, self.description,
                    self.consumerKey, secret, extraneous)
...
class CyverseConnection(object):
            ...
            for key in config:
                if key in synonyms:
                    ...
                    if gotten_value is None:
                        gotten_value = provider._get_config_value(syn)
    ...
    def _verify_client(self):
        ...
        has_info = self._client.consumerKey and self._client.consumerSecret
        return self.create_client(not has_info)
...
class CyverseBucketObject(BaseBucketObject):
    ...
    def __init__(self, provider, obj):
        ...
        self.file_reqs = {
            "filePath": "/{}/".format(provider._get_config_value("filePath")),
    ...
    def _list_local_files(self, path):
        ...
        if isdir(fpath):
            ...
            fullpath_dirs = list(filter(
                lambda x: not _is_subpath(x, fullpath_dirs), fullpath_dirs))
            return (fullpath_dirs, fullpath_files)
        return ([], [fpath])

    def _upload(self, path):
        folders, files = self._list_local_files(path)
